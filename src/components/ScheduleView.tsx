            ...
            <CardContent>
              <div className="overflow-x-auto">
                <div className="min-w-[800px]">
                  ...
                  {siteFields.map(field => (
                    <div key={field.id} className="grid grid-cols-6 gap-2 mb-4">
                      ...
                      {DAYS.map((day, dayIndex) => {
                        ...
                        return (
                          <div key={day} className="relative h-12 bg-muted/30 rounded border border-border">
                            <TooltipProvider>
                              {dayEvents.map(event => {
                                ...
                                return (
                                  <Tooltip key={event.id}>
                                    <TooltipTrigger asChild>
                                      <div
                                        className={`absolute top-1 bottom-1 ${teamColor} rounded px-1 flex items-center justify-center text-white text-xs font-medium cursor-pointer hover:opacity-90 transition-opacity overflow-hidden`}
                                        style={position}
                                      >
                  ...
                  <div className="grid grid-cols-6 gap-2 mt-2 pt-2 border-t">
                    <div></div>
                    {DAYS.map(day => (
                      <div key={day} className="flex justify-between text-[10px] text-muted-foreground px-1">
                        <span>17:00</span>
                        <span>19:30</span>
                        <span>22:00</span>
                      </div>
